...
class _RecordingScreenState extends State<RecordingScreen> {
  ...
  Future<void> _checkPermissions() async {
    // For web, we'll skip permission checks as they're handled differently
      ...
      if (_isRecording) {
        ...
        if (mounted) {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  VideoPreviewScreen(videoPath: videoFile.path),
            ),
          );
        }
      } else {
  ...
  void _switchCamera() async {
    if (_cameraController == null || cameras.length < 2) return;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (_isInitialized && cameras.length > 1)
            IconButton(
